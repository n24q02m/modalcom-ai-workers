...
def test_gguf_convert_model_repo_not_found(mock_hf_hub, mock_env):
    """Test when repo doesn't exist, it should continue and create it."""
...
def test_gguf_convert_model_config_download_fail(mock_hf_hub, mock_env):
    """Test when config files fail to download, it should still succeed."""
    ...
    with (
        ...
        assert result["status"] == "success"
        # hf_hub_download is called for each config file
        assert mock_hf_hub["hf_hub_download"].call_count >= 1
