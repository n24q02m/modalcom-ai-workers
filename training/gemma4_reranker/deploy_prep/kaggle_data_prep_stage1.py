"""Kaggle Stage-1 grouped dataset preparation for Gemma4 reranker."""
...
class GeminiMiner:
    def __init__(self):
        ...
        # BYPASS KAGGLE'S DATA PROXY: Kaggle's proxy causes HTTP 404 for batchEmbedContents
        for k in list(os.environ.keys()):
            if "KAGGLE_DATA_PROXY" in k or "API_BASE_URL" in k:
                del os.environ[k]
...
if __name__ == "__main__":
    ...
    try:
        ...
        for row in ds_marco:
            ...
            positives = [
                text
                for text, is_sel in zip(
                    passages["passage_text"], passages.get("is_selected", []), strict=False
                )
                if is_sel
            ]
            if positives:
        ...
        with open(OUTPUT_PATH, "a", encoding="utf-8") as out_file:
            for i, item in enumerate(tqdm(raw_data[processed_count:], desc="Processing Data")):
                ...
                if len(item["corpus"]) < 15:
                    random_pool = [
                        x["positive"]
                        for x in random.sample(raw_data, min(50, len(raw_data)))
                        if x["modality"] == modality
                    ]
                    corpus = list(dict.fromkeys([*item["corpus"], *random_pool]))
                else:
                    corpus = item["corpus"]
                ...
                try:
                    ...
                    if not isinstance(all_embs, list) or len(all_embs) < 2:
                        raise ValueError("Embedding API returned invalid batch output")
                    ...
                    out_file.write(json.dumps(processed_record, ensure_ascii=False) + "\n")
                    if i % 100 == 0:
                        out_file.flush()
