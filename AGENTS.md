...
- **Static Analysis & Dependencies**: For tools like  to work effectively in CI, dependencies used in imports (even inside methods) should be available in the environment, or explicitly ignored if they are heavy/unavailable (like CUDA-only packages). Adding lightweight dependencies like , ,  to  extras is best practice.
