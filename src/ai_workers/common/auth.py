...
async def auth_middleware(request: Request, call_next):
    """FastAPI middleware for centralized API key authentication.

    Bypasses authentication for ``/health`` and ``/`` endpoints.
    Catches ``HTTPException`` and returns a generic ``JSONResponse``
    consistent with the existing worker implementations.
    """
