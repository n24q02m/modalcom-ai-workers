...
def _resolve_keys() -> list[bytes]:
    ...
    for var in ("API_KEY", "WORKER_API_KEY"):
        ...
        if val and val.encode() not in keys:
            keys.append(val.encode())
...
async def auth_middleware(request: Request, call_next):
    """FastAPI middleware to verify API key for all requests except health checks."""
