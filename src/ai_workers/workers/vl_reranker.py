...
class VLRerankerServer:
    ...
    def _score_pair(
        ...
        if query_image_url:
            ...
            if image_cache is not None and query_image_url in image_cache:
                images.append(image_cache[query_image_url])
            else:
        ...
        if document_image_url:
            ...
            if image_cache is not None and document_image_url in image_cache:
                images.append(image_cache[document_image_url])
            else:
    ...
    def serve(self):
        ...
        async def rerank(body: VLRerankRequest = Body(...)):
            ...
            # Deduplicate and pre-fetch images concurrently
            unique_image_urls = set()
            if body.query_image_url:
                unique_image_urls.add(body.query_image_url)
            for doc in body.documents:
                if not isinstance(doc, str) and doc.image_url:
                    unique_image_urls.add(doc.image_url)

            image_cache = {}
            if unique_image_urls:
                # Use asyncio.to_thread for each URL to avoid blocking the event loop
                urls_list = list(unique_image_urls)
                images = await asyncio.gather(
                    *(asyncio.to_thread(self._load_image, url) for url in urls_list)
                )
                image_cache = dict(zip(urls_list, images, strict=True))
